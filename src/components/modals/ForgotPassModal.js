...
export function ForgotPassModal(props){
    ...
    return(
        <div className="fixed top-0 left-0 w-screen h-screen backdrop-blur-sm z-10 flex justify-center items-center">
            {sendEmail?
            <div className="w-10/12 sm:w-1/2 text-extraSmall md:text-base min-h-[80vh] bg-white border border-gray-700 rounded-md flex justify-around p-4 relative flex-col items-center">
                <div onClick={e=>props.closeModal(false)} className="absolute top-3 right-3 text-base cursor-pointer">X</div>
            ...
            <div className="w-10/12 sm:w-1/2 text-extraSmall md:text-base h-[80vh] bg-white border border-gray-700 rounded-md flex flex-col justify-between items-center p-4 relative">
                <div onClick={e=>props.closeModal(false)} className="absolute top-3 right-3 text-base cursor-pointer">X</div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex flex-col justify-center gap-4 items-center">
                        <div>
                            <input onFocus={e=>setErrorShow(false)} className="w-full sm:w-[25vmax] h-[10vmin] border border-gray-300 pl-2.5 focus:bg-yellow-300" type="email" placeholder="Type your email" onChange={e=>{
                                setForgotPOptions({
                                    ...forgotPOptins,
                        ...
                        }} className="bg-sky-300 w-auto px-2 sm:px-0 sm:w-[8vmax] h-[8vmin] flex items-center justify-center rounded-md text-extraSmall sm:text-xl font-light cursor-pointer text-center">{!isLoading?"SEND":<SaveSpinner/>}</div>
                    </div> 
                </div>
            </div>
            }
